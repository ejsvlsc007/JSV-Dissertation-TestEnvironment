...
# Feature channels at each level: 32→64→128→256→320→320
...
class UNet(nn.Module):
    ...
    Channel flow:
      Stem   : 1   → 32
      Enc 0  : 32  → 64   (stride=2)
      Enc 1  : 64  → 128  (stride=2)
      Enc 2  : 128 → 256  (stride=2)
      Enc 3  : 256 → 320  (stride=2)
      Enc 4  : 320 → 320  (stride=2)  ← bottleneck
      Dec 0  : 320 + skip(320) → 320
      Dec 1  : 320 + skip(256) → 256
      Dec 2  : 256 + skip(128) → 128
      Dec 3  : 128 + skip(64)  → 64
      Dec 4  : 64  + skip(32)  → 32   ← full resolution
      Seg    : 32  → 1  (Conv1x1 + sigmoid)

    Deep supervision aux heads (training only):
      aux[0] : Dec 0 out → ch=320 → 1
      aux[1] : Dec 1 out → ch=256 → 1
      aux[2] : Dec 2 out → ch=128 → 1
      aux[3] : Dec 3 out → ch=64  → 1
    """
    ...
    def __init__(self):
        ...
        # Decoder: D0..D4
        # D0: in=CHANNELS[5]=320, skip=CHANNELS[4]=320, out=CHANNELS[4]=320
        # D1: in=CHANNELS[4]=320, skip=CHANNELS[3]=256, out=CHANNELS[3]=256
        # D2: in=CHANNELS[3]=256, skip=CHANNELS[2]=128, out=CHANNELS[2]=128
        # D3: in=CHANNELS[2]=128, skip=CHANNELS[1]=64,  out=CHANNELS[1]=64
        # D4: in=CHANNELS[1]=64,  skip=CHANNELS[0]=32,  out=CHANNELS[0]=32
        ...
        # Auxiliary heads — must match decoder_outs channel order exactly:
        # decoder_outs[0]=320, [1]=256, [2]=128, [3]=64
        AUX_CHANNELS = [CHANNELS[4], CHANNELS[3], CHANNELS[2], CHANNELS[1]]
        # AUX_CHANNELS = [320, 256, 128, 64]
    ...
    def forward(self, x):
        ...
        # decoder_outs[0]: ch=320  (coarsest)
        # decoder_outs[1]: ch=256
        # decoder_outs[2]: ch=128
        # decoder_outs[3]: ch=64
        # decoder_outs[4]: ch=32  (full resolution)

        # ── Primary prediction ────────────────────────────────────────────────
        primary = torch.sigmoid(self.seg_head(decoder_outs[4]))

        # Eval: plain Tensor — compatible with notebook validate() and metrics
        if not self.training:
            return primary

        # Train: list for deep supervision
        aux_preds = [
...
class DeepSupervisionLoss(nn.Module):
    ...
    Training : weighted sum over all scales (w=1.0, 0.5, 0.25, 0.125, 0.0625)
